...
cJSON *ExtAPI::get_FunJson(const std::string &funName)
{
    ...
    auto ret = cJSON_GetObjectItemCaseSensitive(root, funName.c_str());
    if (ret == nullptr){
        if (funName.find("_ZN5alloc5alloc15exchange_malloc") != std::string::npos){
            ret = cJSON_GetObjectItemCaseSensitive(root, "exchange_malloc");
        }
        // core::alloc::Allocator::allocate
        else if (funName.find("core..alloc..Allocator$GT$8allocate") != std::string::npos){
            ret = cJSON_GetObjectItemCaseSensitive(root, "Allocator::allocate");
        }
        // core::alloc::Allocator::allocate_zeroed
        else if (funName.find("core..alloc..Allocator$GT$15allocate_zeroed") != std::string::npos){
            ret = cJSON_GetObjectItemCaseSensitive(root, "Allocator::allocate");
        }
        // core::alloc::Allocator::deallocate
        else if (funName.find("core..alloc..Allocator$GT$10deallocate") != std::string::npos){
            ret = cJSON_GetObjectItemCaseSensitive(root, "Allocator::deallocate");
        }
        // // core::alloc::Allocator::grow
        // else if (funName.find("_ZN5alloc5alloc15exchange_malloc") != std::string::npos){
        ...
        // }
        if (funName.find("core..alloc..global..GlobalAlloc") != std::string::npos){
            if (funName.find("realloc") != std::string::npos){
                ret = cJSON_GetObjectItemCaseSensitive(root, "GllobalAlloc::realloc");
            }
            else if (funName.find("dealloc") != std::string::npos){
                ret = cJSON_GetObjectItemCaseSensitive(root, "GllobalAlloc::dealloc");
            }
            else if (funName.find("alloc_zeroed") != std::string::npos){
                ret = cJSON_GetObjectItemCaseSensitive(root, "GllobalAlloc::alloc");
            }
            else if (funName.find("5alloc") != std::string::npos) {
                ret = cJSON_GetObjectItemCaseSensitive(root, "GllobalAlloc::alloc");
            }
        }
    }
...
bool ExtAPI::is_ext(const SVFFunction *F)
{
    ...
    else
    {
        ...
        //if (fname.find("_ZN72_$LT$alloc..vec..Vec$LT$T$C$A$GT$$u20$as$u20$core..ops..deref..Deref$GT$5deref") != std::string::npos ||
        if (fname.find("core..ops..deref..Deref$GT$5deref") != std::string::npos ||
            // fname.find("_ZN81_$LT$alloc..vec..Vec$LT$T$C$A$GT$$u20$as$u20$core..ops..index..Index$LT$I$GT$$GT$5index") != std::string::npos) {
            fname.find("core..ops..index..Index$LT$I$GT$$GT$5index") != std::string::npos ||
            fname.find("core..clone..Clone$GT$5clone") != std::string::npos
        ) {
            res = 1;
            return res;
        }
