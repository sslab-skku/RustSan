...
void SVFIRBuilder::visitInsertValueInst(InsertValueInst &inst) {
    ...
    NodeID src2 = getValueNode(inst.getAggregateOperand());
...
void SVFIRBuilder::visitSwitchInst(SwitchInst &inst)
{
    ...
    for (u32_t i = 0; i < inst.getNumSuccessors(); ++i)
    {
        ...
        else if (condVal && condVal->getBitWidth() == 128)
            val = condVal->getValue().	getLimitedValue();
        const ICFGNode* icfgNode = pag->getICFG()->getICFGNode(succInst);
...
void SVFIRBuilder::handleExtCall(CallSite cs, const SVFFunction *callee)
{
    ...
    else
    {
        if (isExtCall(callee))
        {
            ...
            if (funName.find("core..ops..deref..Deref$GT$5deref") != std::string::npos) {
                NodeID retNode = getValueNode(inst);
                NodeID srcNode = getValueNode(inst->getOperand(0));
                addLoadEdge(srcNode, retNode);
            } else if (funName.find("core..ops..index..Index$LT$I$GT$$GT$5index") != std::string::npos) {
                NodeID retNode = getValueNode(inst);
                NodeID srcNode = getValueNode(inst->getOperand(0));
                addCopyEdge(srcNode, retNode);
            } else if (funName.find("core..clone..Clone$GT$5clone") != std::string::npos) {
                if (inst->getNumOperands() == 2) {
                    NodeID dstNode = getValueNode(inst->getOperand(0));
                    NodeID srcNode = getValueNode(inst->getOperand(1));
