...
struct Allocator {
  ...
  void *Allocate(uptr size, uptr alignment, BufferedStackTrace *stack,
                 AllocType alloc_type, bool can_fill, bool safety = false) {
    //Printf("\x1b[1;34mAllocate called: size = %u, safety = %d\n", size, safety);
    ...
    m->SetAllocContext(t ? t->tid() : kMainTid, StackDepotPut(*stack));
    m->is_safe_obj = safety;

    uptr size_rounded_down_to_granularity =
        RoundDownTo(size, SHADOW_GRANULARITY);
    // Unpoison the bulk of the memory region.
    if (size_rounded_down_to_granularity)
      PoisonShadow(user_beg, size_rounded_down_to_granularity, (safety) ? 0x10 : 0x00);
    // Deal with the end of the region if size is not aligned to granularity.
    if (size != size_rounded_down_to_granularity && CanPoisonMemory()) {
      u8 *shadow =
          (u8 *)MemToShadow(user_beg + size_rounded_down_to_granularity);
      *shadow = fl.poison_partial ? ((size & (SHADOW_GRANULARITY - 1)) | ((safety) ? 0x10 : 0x00)) : 0;
    }
    ...
    //Printf("\x1b[1;33mallocated: %p, size: %u, safety: %d\x1b[0m\n", allocated, size, safety);
    ASAN_MALLOC_HOOK(res, size);
    //Printf("\x1b[1;33mASAN_MALLOC_HOOK: %p\x1b[0m\n", res);
    return res;
  }
  ...
  void Deallocate(void *ptr, uptr delete_size, uptr delete_alignment,
    ...
    if (UNLIKELY(m->is_safe_obj && from_untrusted)) {
      ReportFreeRustObjectFromUntrusted(p, stack);
    }
  ...
  void *Reallocate(void *old_ptr, uptr new_size, BufferedStackTrace *stack) {
    ...
    bool safety = m->is_safe_obj;

    AsanStats &thread_stats = GetCurrentThreadStats();
    thread_stats.reallocs++;
    thread_stats.realloced += new_size;

    void *new_ptr = Allocate(new_size, 8, stack, FROM_MALLOC, true, safety);
    if (new_ptr) {
      ...
      Deallocate(old_ptr, 0, 0, stack, FROM_MALLOC, false);
    }
    return new_ptr;
  }

  void *Calloc(uptr nmemb, uptr size, BufferedStackTrace *stack, bool safety) {
    if (UNLIKELY(CheckForCallocOverflow(size, nmemb))) {
      if (AllocatorMayReturnNull())
        return nullptr;
      ReportCallocOverflow(nmemb, size, stack);
    }
    void *ptr = Allocate(nmemb * size, 8, stack, FROM_MALLOC, false, safety);
...
void asan_free(void *ptr, BufferedStackTrace *stack, AllocType alloc_type,
               bool from_untrusted) {
  instance.Deallocate(ptr, 0, 0, stack, alloc_type, from_untrusted);
}
...
void *asan_malloc(uptr size, BufferedStackTrace *stack, bool safety) {
  return SetErrnoOnNull(
      instance.Allocate(size, 8, stack, FROM_MALLOC, true, safety));
}

void *asan_calloc(uptr nmemb, uptr size, BufferedStackTrace *stack,
                  bool safety) {
  return SetErrnoOnNull(instance.Calloc(nmemb, size, stack, safety));
}
...
void *asan_realloc(void *p, uptr size, BufferedStackTrace *stack, bool safety) {
  if (!p)
    return SetErrnoOnNull(
        instance.Allocate(size, 8, stack, FROM_MALLOC, true, safety));  // TODO: get safety from code...
  if (size == 0) {
    if (flags()->allocator_frees_and_returns_null_on_realloc_zero) {
      instance.Deallocate(p, 0, 0, stack, FROM_MALLOC, /*!safety*/ false);
      return nullptr;
    }
...
void *asan_memalign(uptr alignment, uptr size, BufferedStackTrace *stack,
                    AllocType alloc_type, bool safety) {
  ...
  return SetErrnoOnNull(
      instance.Allocate(size, alignment, stack, alloc_type, true, safety));
}
...
int asan_posix_memalign(void **memptr, uptr alignment, uptr size,
                        BufferedStackTrace *stack, bool safety) {
  if (UNLIKELY(!CheckPosixMemalignAlignment(alignment))) {
    if (AllocatorMayReturnNull())
      return errno_EINVAL;
    ReportInvalidPosixMemalignAlignment(alignment, stack);
  }
  void *ptr = instance.Allocate(size, alignment, stack, FROM_MALLOC, true, safety);
  if (UNLIKELY(!ptr))
...
// FIXME: Test Function
void __asan_marking_heap_safe(void const volatile *addr, bool safety) {
