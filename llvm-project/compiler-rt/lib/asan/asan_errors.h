...
struct ErrorFreeRustObjectFromUntrusted : ErrorBase {
  const BufferedStackTrace *free_stack;
  HeapAddressDescription addr_description;

  ErrorFreeRustObjectFromUntrusted() = default;  // (*)
  ErrorFreeRustObjectFromUntrusted(u32 tid, BufferedStackTrace *stack,
                                   uptr addr)
      : ErrorBase(tid, 42, "free-from-untrusted"), free_stack(stack) {
...
#define ASAN_FOR_EACH_ERROR_KIND(macro)         \
  macro(FreeRustObjectFromUntrusted)            \
  macro(DeadlySignal)                           \
  macro(DoubleFree)                             \
