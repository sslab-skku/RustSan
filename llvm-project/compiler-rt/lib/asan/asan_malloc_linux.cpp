...
u64* __shadow_hit_counter;
u64 __get_shadow_hit_counter() {
    if (__shadow_hit_counter == nullptr)
        return -1;
...
void __init_shadow_hit_counter() {
    if (__shadow_hit_counter == nullptr)
        __shadow_hit_counter = (u64*)__asan_rust_malloc(8);
    *__shadow_hit_counter = 0;
}

void* __asan_rust_malloc(uptr size) {
  if (UNLIKELY(UseLocalPool())) {
  ...
  ENSURE_ASAN_INITED();
  GET_STACK_TRACE_MALLOC;
  return asan_malloc(size, &stack, true);
  //printf("__asan_rust_malloc in asan: %p\n", *result);
}

void* __asan_rust_realloc(void *ptr, uptr size) {
  ...
  ENSURE_ASAN_INITED();
  GET_STACK_TRACE_MALLOC;
  return asan_realloc(ptr, size, &stack, true);
}

void* __asan_rust_calloc(uptr nmemb, uptr size) {
  if (UNLIKELY(UseLocalPool())) {
  ...
  ENSURE_ASAN_INITED();
  GET_STACK_TRACE_MALLOC;
  return asan_calloc(nmemb, size, &stack, true);
}

void __asan_rust_free(void *ptr) {
  if (UNLIKELY(IsInDlsymAllocPool(ptr))) {
    DeallocateFromLocalPool(ptr);
    return;
  }
  GET_STACK_TRACE_FREE;
  asan_free(ptr, &stack, FROM_MALLOC, false);
}

void *__asan_rust_memalign(uptr boundary, uptr size) {
  GET_STACK_TRACE_MALLOC;
  return asan_memalign(boundary, size, &stack, FROM_MALLOC, true);
}

int __asan_rust_posix_memalign(void **memptr, uptr alignment, uptr size) {
  if (UNLIKELY(UseLocalPool()))
    return PosixMemalignFromLocalPool(memptr, alignment, size);
  GET_STACK_TRACE_MALLOC;
  return asan_posix_memalign(memptr, alignment, size, &stack, true);
}
...
INTERCEPTOR(void, free, void *ptr) {
  ...
  GET_STACK_TRACE_FREE;
  asan_free(ptr, &stack, FROM_MALLOC, true);
}
