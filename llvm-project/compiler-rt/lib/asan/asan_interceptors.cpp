...
extern "C" {
extern __attribute__((visibility(
    "default"))) __thread void *__safestack_unsafe_stack_ptr = nullptr;
}

__thread void *unsafe_stack_start = nullptr;

inline void unsafe_stack_setup(void *start, uptr size) {
  void *stack_ptr = (char *)start + size;
  __safestack_unsafe_stack_ptr = stack_ptr;
}

inline void *safe_stack_alloc(uptr size) {
  // void *addr =
  //     (void *)MmapNamed(nullptr, size + guard, 0x1 | 0x2, 0x2 | 0x20,
  //     nullptr);
  // internal_mprotect(addr, guard, 0x0);
  // return (char *)addr + guard;
  void *addr =
      (void *)internal_mmap(nullptr, size, 0x1 | 0x2, 0x2 | 0x20, -1, 0);
  return (char *)addr;
}
...
__safestack_init() {
  ...
  void *addr = safe_stack_alloc(size);
  unsafe_stack_setup(addr, size);
...
static thread_return_t THREAD_CALLING_CONV asan_thread_start(void *arg) {
  ...
  SetCurrentThread(t);
  if (t->safestack_size != 0) {
    void *addr = safe_stack_alloc(t->safestack_size);
    unsafe_stack_setup(addr, t->safestack_size);
  }
...
INTERCEPTOR(int, pthread_create, void *thread,
  ...
  AsanThread *t = AsanThread::Create(start_routine, arg, current_tid, &stack,
                                     detached, size);
  // ThreadArgs args = {t, addr, size, guard};
