...
void ReportFreeRustObjectFromUntrusted(uptr addr,
                                       BufferedStackTrace *free_stack) {
  ScopedInErrorReport in_report;
  ErrorFreeRustObjectFromUntrusted error(GetCurrentTidOrInvalid(), free_stack,
                                         addr);
...
void ReportGenericError(uptr pc, uptr bp, uptr sp, uptr addr, bool is_write,
  ...
  // FIXME: Crazy hack! fix for aa 00 mixed case
  if (access_size <= 64 && access_size >= 16) {
    // Maybe FP!
    return;
    const char *shadow_beg = (const char *)MemToShadow(addr);
    const char *shadow_end = (const char *)MemToShadow(addr + access_size);
    u8 mem = 0;
    for (; shadow_beg < shadow_end; ++shadow_beg) {
      mem |= *shadow_beg;
    }
    if (mem == 0xaa)
      return;
  } else if (access_size == 8) {
    // FIXME: Really crazy hack!!!!
    const unsigned char *shadow_beg = (const unsigned char *)MemToShadow(addr);
    if (*shadow_beg == 0 || *shadow_beg == 0xaa)
      return;
  }
