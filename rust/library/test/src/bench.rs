...
#[link(name = "clang_rt.asan", kind = "static")]
extern {
    fn __get_shadow_hit_counter() -> libc::c_ulonglong;
    fn __init_shadow_hit_counter();
}

fn init_shadow_hit_counter() {
    unsafe { __init_shadow_hit_counter();}
}

fn get_shadow_hit_counter() -> u64 {
    unsafe {__get_shadow_hit_counter() as u64}
}

fn get_and_init_shadow_hit_counter() ->u64 {
    let counter = get_shadow_hit_counter();
    init_shadow_hit_counter();
    counter
}
...
pub fn fmt_bench_samples(bs: &BenchSamples) -> String {
    ...
    write!(output, ", Shadow Count = {}", bs.ns_iter_summ.shadow_count).unwrap();
    output
}
    ...
    F: FnMut() -> T,
{
    ...
    let counter = get_and_init_shadow_hit_counter();
    ...
    loop {
        ...
        stats::winsorize(samples, 5.0);
        let mut summ5 = stats::Summary::new(samples);
        summ5.shadow_count = counter;
