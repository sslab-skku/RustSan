...
const char kAsanAllocaColoring[] = "__asan_marking_memory_region_with_value";
const unsigned char kAsanRustBenignMagic = 0x10;
...
// Command-line flags.
static cl::opt<bool> ClShadowCounter(
    "shadow-counter",
    cl::desc(
        "Count shadow memory hit number"),
    cl::Hidden, cl::init(false));

static cl::opt<bool> ClRustInstrumentation(
    "asan-instrument-rust",
    cl::desc(
        "Instrumentation for Rust like allowing access to Magic Value(0xaa)"),
    cl::Hidden, cl::init(false));

static cl::opt<bool> ClRustSafeStack(
    "asan-rust-safestack",
    cl::desc(
        "Instrumentation for Rust like allowing access to Magic Value(0xaa)"),
    cl::Hidden, cl::init(false));

static cl::opt<bool> ClDebloatingAsan(
    "debloating-asan",
    cl::desc(
        "debloating-asan implementation"),
    cl::Hidden, cl::init(false));
...
static cl::opt<bool>
    ClOptStack("asan-opt-stack",
               ...
               cl::init(true)); //ASAN--: Removing Unsatisfiable Checks
...
struct FunctionStackPoisoner : public InstVisitor<FunctionStackPoisoner> {
  ...
  SmallVector<AllocaInst *, 16> SafeAllocaVec;
  ...
  FunctionCallee AsanAllocaColoringFunc;
  ...
  bool runOnFunction() {
    ...
    if (AllocaVec.empty() && DynamicAllocaVec.empty() && SafeAllocaVec.empty())
      return false;
  ...
  void visitAllocaInst(AllocaInst &AI) {
    ...
    if (!AI.isStaticAlloca())
      ...
      if (ClRustSafeStack) {
        if (!AI.hasMetadata(kUnsafeMetadata))
          SafeAllocaVec.push_back(&AI);
        else
...
static void instrumentMaskedLoadOrStore(AddressSanitizer *Pass,
  ...
  for (unsigned Idx = 0; Idx < Num; ++Idx) {
    ...
    if (auto *Vector = dyn_cast<ConstantVector>(Mask)) {
      ...
      ThenTerm->setMetadata("nosancov", MDNode::get(IRB.getContext(), None));
      I->setMetadata("nosancov", MDNode::get(IRB.getContext(), None));
...
void AddressSanitizer::instrumentMop(ObjectSizeOffsetVisitor &ObjSizeVis,
  ...
  Value *Addr = O.getPtr();
  if (!ClDebloatingAsan) {
    instrumentMopLoop(ObjSizeVis, O, nullptr, UseCalls, DL);
    return;
  }
...
void AddressSanitizer::instrumentMopLoop(ObjectSizeOffsetVisitor &ObjSizeVis,
  ...
  if (O.MaybeMask) {
    ...
    Instruction* PI = ClDebloatingAsan? PrevI : O.getInsn();
    doInstrumentAddress(this, O.getInsn(), PI, Addr, O.Alignment,
                        Granularity, O.TypeSize, O.IsWrite, nullptr, UseCalls,
                        Exp);
  }
}
...
Instruction *AddressSanitizer::instrumentAMDGPUAddress(
  ...
  InsertBefore->setMetadata("nosancov", MDNode::get(IRB.getContext(), None));
...
void AddressSanitizer::instrumentAddress(Instruction *OrigIns,
  ...
  if(ClShadowCounter){
    Constant* Counter = OrigIns->getParent()->getParent()->getParent()->getOrInsertGlobal("__shadow_hit_counter", Type::getInt64PtrTy(IRB.getContext(), 0));
    Value* CounterPtr = IRB.CreateLoad(Type::getInt64PtrTy(IRB.getContext(), 0), Counter);
    IRB.CreateStore(IRB.CreateAdd(IRB.CreateLoad(IRB.getInt64Ty(), CounterPtr), IRB.getInt64(1)), CounterPtr);
  }
  // IRB.CreateCondBr()
  // SplitBlockAndInsertIfThen()
  
  if (ClRustInstrumentation) {
    // Value *PtrCmp = IRB.CreateICmpUGT(IRB.CreatePtrToInt(Addr, IRB.getInt64Ty()), IRB.getInt64(0x7ffcf091bbf8));
    do{
      if(Instruction* I = dyn_cast<Instruction>(Addr)){
        if(I->hasMetadata(kUnsafeMetadata))
          break;
      }
      Constant *MaskValue = nullptr;
      long magic8 = kAsanRustBenignMagic;
      long magic16 = magic8 << 8 | magic8;
      long magic32 = magic16 << 16 | magic16;
      long magic64 = magic32 << 32 | magic32;
      switch (TypeSize >> Mapping.Scale) {
      default:
      case 8:
        MaskValue = ConstantInt::get(ShadowTy, ~magic8);
        break;
      case 16:
        MaskValue = ConstantInt::get(ShadowTy, ~magic16);
        break;
      case 32:
        MaskValue = ConstantInt::get(ShadowTy, ~magic32);
        break;
      case 64:
        MaskValue = ConstantInt::get(ShadowTy, ~magic64);
        break;
      }
      // Value* MaskedShadowValue = IRB.CreateAnd(ShadowValue, MaskValue);
      // ShadowValue = IRB.CreateSelect(PtrCmp, MaskedShadowValue, ShadowValue);
    } while(false);
  }
  ...
  if (ClAlwaysSlowPath || (TypeSize < 8 * Granularity)) {
    ...
    CheckTerm->setMetadata("nosancov", MDNode::get(IRB.getContext(), None));
    InsertBefore->setMetadata("nosancov", MDNode::get(IRB.getContext(), None));
  ...
  CrashTerm->setMetadata("nosancov", MDNode::get(IRB.getContext(), None));
...
bool ModuleAddressSanitizer::instrumentModule(Module &M) {
  ...
  // (2) target is ELF.
  errs() << "I'm using Priority as" << Priority << "!!\n";
...
void AddressSanitizer::optimizeInstrumentation(Function &F, std::list<std::pair<int, std::pair<std::pair<int64_t, llvm::Instruction *>, std::vector<std::pair<int64_t, llvm::Instruction *>>>>> &rankPotentialRemoveInsts, std::set<Instruction *> &deleted, SmallVector<InterestingMemoryOperand, 16> &OperandsToInstrument) {
  ...
  for (auto optInst = rankPotentialRemoveInsts.begin(); optInst != rankPotentialRemoveInsts.end(); ++optInst) {
    ...
    CheckTerm->setMetadata("nosancov", MDNode::get(IRB.getContext(), None));
    lastInst.second->setMetadata("nosancov", MDNode::get(IRB.getContext(), None));
...
void AddressSanitizer::InvariantOptimizeHandler(Loop *L, std::set<Instruction *> &optimized, 
  ...
  if (DT.dominates(Oper.getInsn(), ExitBB)) {
    ...
    CheckTerm->setMetadata("nosancov", MDNode::get(IRBcheck.getContext(), None));
    exitInst->setMetadata("nosancov", MDNode::get(IRBcheck.getContext(), None));
...
void AddressSanitizer::MonotonicOptimizeHandler(Loop *L, std::set<Instruction *> &optimized, 
  ...
  CheckTerm->setMetadata("nosancov", MDNode::get(IRBinsertCheck.getContext(), None));
  Inst->setMetadata("nosancov", MDNode::get(IRBinsertCheck.getContext(), None));
  ...
  if (DT.dominates(Inst, ExitBB)) {
    ...
    ExitCheckTerm->setMetadata("nosancov", MDNode::get(IRBreChk.getContext(), None));
    exitInst->setMetadata("nosancov", MDNode::get(IRBreChk.getContext(), None));
    if ((TypeSize == 8 || TypeSize == 16 || TypeSize == 32 || TypeSize == 64 || TypeSize == 128) && (!Alignment || *Alignment >= Granularity || *Alignment >= TypeSize / 8)) {
...
void FunctionStackPoisoner::initializeCallbacks(Module &M) {
  ...
  AsanAllocaColoringFunc = M.getOrInsertFunction(
      kAsanAllocaColoring, IRB.getVoidTy(), IntptrTy, IntptrTy);
...
void FunctionStackPoisoner::processSafeStaticAllocas() {
  if (SafeAllocaVec.empty())
    return;

  if (!ClRustSafeStack)
    assert(false);

  Instruction *InsBefore = SafeAllocaVec[0];
  IRBuilder<> IRB(InsBefore);
  Module *M = IRB.GetInsertBlock()->getParent()->getParent();

  // We use unsafe stack on safe stack as our safe stack base.
  const char *SafeStackPtrVar = "__safestack_unsafe_stack_ptr";
  auto SafeStackPtr =
      dyn_cast_or_null<GlobalVariable>(M->getNamedValue(SafeStackPtrVar));
  Type *StackPtrTy = Type::getInt8PtrTy(M->getContext());
  if (!SafeStackPtr) {
    // The global variable is not defined yet, define it ourselves.
    // We use the initial-exec TLS model because we do not support the
    // variable living anywhere other than in the main executable.
    // errs() << "Nah, I can't find the __safestack_unsafe_stack_ptr...\n";
    SafeStackPtr = new GlobalVariable(
        *M, StackPtrTy, false, GlobalValue::ExternalLinkage, nullptr,
        SafeStackPtrVar, nullptr, GlobalValue::InitialExecTLSModel);
  }
  Value *BasePointer = IRB.CreatePtrToInt(
      IRB.CreateLoad(StackPtrTy, SafeStackPtr, false, "safe_stack_ptr"),
      IntptrTy);

  for (AllocaInst *AI : SafeAllocaVec) {
  ...
  Constant *SafeStackSize = ConstantInt::get(IntptrTy, L.FrameSize);
  Value *BasePtrtoSub = IRB.CreateSub(BasePointer, SafeStackSize);
  IRB.CreateStore(IRB.CreateIntToPtr(BasePtrtoSub, StackPtrTy), SafeStackPtr);

  for (const auto &Desc : SVD) {
    AllocaInst *AI = Desc.AI;
    Value *NewAllocaPtr = IRB.CreateIntToPtr(
        IRB.CreateAdd(BasePtrtoSub, ConstantInt::get(IntptrTy, Desc.Offset)),
        AI->getType());
    AI->replaceAllUsesWith(NewAllocaPtr);
  }
  // Create pseudo-epilogue for every return instructions
  for (Instruction *Ret : RetVec) {
    IRBuilder<> IRBRet(Ret);
    IRBRet.CreateStore(IRB.CreateIntToPtr(BasePointer, StackPtrTy),
                       SafeStackPtr);
  }
}
void FunctionStackPoisoner::processStaticAllocas() {
  ...
  if (DoStackMalloc) {
    ...
    if (ASan.UseAfterReturn == AsanDetectStackUseAfterReturnMode::Runtime) {
      ...
      Term->setMetadata("nosancov", MDNode::get(IRB.getContext(), None));
      InsBefore->setMetadata("nosancov", MDNode::get(IRB.getContext(), None));
  ...
  for (Instruction *Ret : RetVec) {
    ...
    if (DoStackMalloc) {
      ...
      ThenTerm->setMetadata("nosancov", MDNode::get(IRBRet.getContext(), None));
      ElseTerm->setMetadata("nosancov", MDNode::get(IRBRet.getContext(), None));
...
void FunctionStackPoisoner::handleDynamicAllocaCall(AllocaInst *AI) {
  ...
  // FIXME: This code is not tested because test rust code seems not having
  // dynamic alloca
  //        So I'm not sure this coloring work or not...
  if (ClRustInstrumentation) {
    if (AI->hasMetadata(kUnsafeMetadata))
      IRB.CreateCall(AsanAllocaPoisonFunc, {NewAddress, OldSize});
    else {
      Value *Size = IRB.CreateFDiv(OldSize, ConstantInt::get(IntptrTy, 8));
      IRB.CreateCall(AsanAllocaColoringFunc,
                     {ASan.memToShadow(NewAddress, IRB), Size});
    }
  } else {
    IRB.CreateCall(AsanAllocaPoisonFunc, {NewAddress, OldSize});
  }
