  ...
  if (GlobalObject *GV = dyn_cast<GlobalObject>(P))
    ...
    if (Instruction* I = dyn_cast<Instruction>(P)){
      switch (I->getOpcode()){
        ...
        case Instruction::UAlloca: {
          UAllocaInst *AI = static_cast<UAllocaInst*>(I);
          AI->setAlignment(Align(Bytes));
          break;
        }
      }
...
LLVMValueRef LLVMBuildUAlloca(LLVMBuilderRef B, LLVMTypeRef Ty,
                             const char *Name) {
  return wrap(unwrap(B)->CreateUAlloca(unwrap(Ty), nullptr, Name));
}
