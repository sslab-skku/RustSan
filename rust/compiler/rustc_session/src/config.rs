...
pub fn rustc_short_optgroups() -> Vec<RustcOptGroup> {
    vec![
        ...
        opt::flag_s("", "unsafe-mir-analysis", "Mark arg/ret by MIR analysis"),
        opt::flag_s("", "mark-alloc", "Mark allocation function"),
        opt::flag_s("", "pta-filter", "Filtering instructions that does not require PTA"),
        opt::flag_s("", "mydbg", "RustSan; enable pass for debugging/statistic purpose"),
        opt::flag_s("", "phase1", "MIR analysis; phase 1: emit the result from intra-crate analysis"),
        opt::flag_s("", "phase2", "MIR analysis; phase 2: inter-crate analysis and mark unsafe allocs"),
    ]
}
...
pub fn build_session_options(matches: &getopts::Matches) -> Options {
    ...
    if unstable_opts.instrument_mcount {
        cg.passes.push("ee-instrument<post-inline>".to_string());
    }
    ...
    let unsafe_mir = matches.opt_present("unsafe-mir-analysis");
    let mark_alloc = matches.opt_present("mark-alloc");
    let pta_filter = matches.opt_present("pta-filter");
    let mydbg = matches.opt_present("mydbg");
    let phase1 = matches.opt_present("phase1");
    let phase2 = matches.opt_present("phase2");
