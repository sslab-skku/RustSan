...
use rustc_middle::ty::{self, Instance, Ty, TypeFoldable, TypeVisitable, InstanceDef, DefIdTree, TyKind};
...
pub fn codegen_mir<'a, 'tcx, Bx: BuilderMethods<'a, 'tcx>>(
    ...
    if let Some(impl_id) = cx.tcx().impl_of_method(instance.def_id()){
        if cx.tcx().sess.opts.phase2 {
            for predicate in cx.tcx().predicates_of(impl_id).predicates {
                let p: rustc_middle::ty::Binder<'tcx, rustc_middle::ty::PredicateKind<'tcx>> = predicate.0.kind();
                if let ty::PredicateKind::Trait(t) = p.skip_binder() {
                    if format!("{:?}", t.trait_ref).contains("std::alloc::Allocator") {
                        // println!("[COMPILER] THIS METHOD IS CHOOSEN :{:?}!!!", instance.def_id());
                        bx.set_allocate_method_attribute();
                    }
                }
            }
        }
    ...
    fx.locals = {
        ...
        let mut allocate_local = |local| {
            ...
            bx.set_safety(!decl.safety);
//            println!("{:?}: {}", decl, decl.safety);
            ...
            if local == mir::RETURN_PLACE && fx.fn_abi.ret.is_indirect() {
                ...
                let result = LocalRef::Place(PlaceRef::new_sized(llretptr, layout));
                bx.set_safety(false);
                return result;
            }
            ...
            if memory_locals.contains(local) {
                ...
                if pta_filter {
                    // println!("accessing local safety: {:?} {:?} ({})", fx.mir.did, local, fx.mir.local_decls[local].safety);
                    unsafe { to_mut(&fx.mir.local_decls[local]).is_allocated = true; }
                }
                if layout.is_unsized() {
                    result = LocalRef::UnsizedPlace(PlaceRef::alloca_unsized_indirect(&mut bx, layout))
                } else {
                    result = LocalRef::Place(PlaceRef::alloca(&mut bx, layout))
                }
            } else {
                debug!("alloc: {:?} -> operand", local);
                result = LocalRef::new_operand(&mut bx, layout)
            }
        ...
        if pta_filter {
        //     println!("fx locals {:?}: {:?}", fx.mir.did, memory_locals);
        }
