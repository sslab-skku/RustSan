...
use rustc_middle::mir::{self, AssertKind, SwitchTargets, SourceScope};
...
impl<'a, 'tcx> TerminatorCodegenHelper<'tcx> {
    ...
    fn do_call<Bx: BuilderMethods<'a, 'tcx>>(
        ...
        if let Some(unwind_block) = unwind_block {
            ...
            /*
            if bx.cx().tcx().sess.opts.mydbg {
                if bx.safety() == true {
                    println!("caller: {:?}, safety: {}, invokeret{:?}", fx.mir.did, bx.safety(), invokeret);
                }
            }
            */
...
impl<'a, 'tcx, Bx: BuilderMethods<'a, 'tcx>> FunctionCx<'a, 'tcx, Bx> {
    ...
    fn codegen_call_terminator(
        ...
        // FIXME: unify the compile-time flag!
        if bx.cx().tcx().sess.opts.unsafe_to_ir {
            let unsafe_args_vec = &self.mir.unsafe_locals;
            let mut idx_vec : Vec<u32> = Vec::<u32>::new();
            for (idx, arg) in (0u32..).zip(args.into_iter()) {
                if let Some(place) = arg.place() {
                    if let Some(local) = place.as_local() {
                        let unsafety:bool = unsafe_args_vec.into_iter().map(|x|{
                            return *x == local
                        ...
                        // TODO: projection implementation
                    }
                }
            }
            let x : u32 = idx_vec.into_iter().map(|idx| 1 << idx).fold(0, |acc, x| acc + x);
            bx.set_unsafe_args(x);
        }
...
impl<'a, 'tcx, Bx: BuilderMethods<'a, 'tcx>> FunctionCx<'a, 'tcx, Bx> {
    fn is_unsafe_scope(&mut self, _scope: &Option<SourceScope>)->bool{
        let mut scope = _scope;
        let safety = loop {
            if scope.is_none() {
                break false;
            } else {
                if let rustc_middle::mir::ClearCrossCrate::Set(source_scope_local_data) = &self.mir.source_scopes[scope.unwrap()].local_data {
                    if source_scope_local_data.safety != rustc_middle::mir::Safety::Safe {
                        break true;
                    }
                }
                scope = &self.mir.source_scopes[scope.unwrap()].parent_scope;
            }
        };
        safety
    }
    ...
    /*
    pub fn add_function_attribute(&mut self) {
        let attr = 
        Fn: &'a Value,
        index: c_uint,
    ...
    */
    ...
    pub fn codegen_block(&mut self, bb: mir::BasicBlock) {
        ...
        let pta_filter = bx.cx().tcx().sess.opts.pta_filter;
        let unsafe_to_ir = bx.cx().tcx().sess.opts.unsafe_to_ir;

        for statement in &data.statements {
            let mut unsafe_or_not = false;
            /*
            if bx.cx().tcx().sess.opts.unsafe_to_ir {
                if let rustc_middle::mir::ClearCrossCrate::Set(source_scope_local_data) = &self.mir.source_scopes[statement.source_info.scope].local_data {
                    if source_scope_local_data.safety == rustc_middle::mir::Safety::Safe {
                        unsafe_or_not = self.is_unsafe_scope(&self.mir.source_scopes[statement.source_info.scope].parent_scope);
                    } else {
                        unsafe_or_not = true;
                    }
                } else { unsafe_or_not = self.is_unsafe_scope(&self.mir.source_scopes[statement.source_info.scope].parent_scope); };
            }
            */

            if pta_filter || unsafe_to_ir {
                unsafe_or_not = statement.pta_target;
            }

            // kimjy: unsafe_or_not: TRUE if UNSAFE
            if unsafe_or_not && pta_filter {
                // pta filtering
                ...
                if locals.is_some() {
                    for local in locals.unwrap() {
                        if mir.local_decls[local].is_allocated == false {
                ...
                unsafe_or_not = !is_safe_to_filter;
            }
        ...
        bx.set_safety(false);
        // FIXME: add safety check function
        let terminator = data.terminator();
        let mut unsafe_or_not = false;
        /*
        if bx.cx().tcx().sess.opts.unsafe_to_ir {
            if let rustc_middle::mir::ClearCrossCrate::Set(source_scope_local_data) = &self.mir.source_scopes[terminator.source_info.scope].local_data {
                if source_scope_local_data.safety == rustc_middle::mir::Safety::Safe {
                    unsafe_or_not = self.is_unsafe_scope(&self.mir.source_scopes[terminator.source_info.scope].parent_scope);
                } else {
                    unsafe_or_not = true;
                }
            } else { unsafe_or_not = self.is_unsafe_scope(&self.mir.source_scopes[terminator.source_info.scope].parent_scope); };
        }
        */

        if pta_filter || unsafe_to_ir {
            unsafe_or_not = terminator.pta_target;
        }

        if bx.cx().tcx().sess.opts.mydbg {
            println!("(1) unsafe_or_not {}", unsafe_or_not);
        }
        ...
        if bx.cx().tcx().sess.opts.mydbg {
            println!("(2) unsafe_or_not {}", unsafe_or_not);
        }
        bx.set_safety(unsafe_or_not);
    ...
    fn codegen_terminator(
        ...
        match terminator.kind {
            ...
            mir::TerminatorKind::Call {
                ...
                if bx.cx().tcx().sess.opts.mydbg {
                    println!("terminator: {:?}, scope: {:?}, safety: {}", terminator, terminator.source_info.scope, bx.safety());
                }
