...
impl<'tcx> CFG<'tcx> {
    ...
    pub(crate) fn push_assign(
        ...
        self.push(
            block,
            Statement { source_info, kind: StatementKind::Assign(Box::new((place, rvalue))), pta_target: false },
        );
    }
    ...
    pub(crate) fn push_fake_read(
        ...
        let stmt = Statement { source_info, kind, pta_target: false };
    ...
    pub(crate) fn terminate(
        ...
        self.block_data_mut(block).terminator = Some(Terminator { source_info, kind, pta_target: false });
    }
