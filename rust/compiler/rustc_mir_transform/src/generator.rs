...
fn insert_switch<'tcx>(
    ...
    body.basic_blocks_mut().raw.insert(
        ...
        BasicBlockData {
            ...
            terminator: Some(Terminator { source_info, kind: switch, pta_target: false }),
            is_cleanup: false,
        },
    );
...
fn elaborate_generator_drops<'tcx>(tcx: TyCtxt<'tcx>, body: &mut Body<'tcx>) {
    ...
    for (block, block_data) in body.basic_blocks.iter_enumerated() {
        let (target, unwind, source_info) = match block_data.terminator() {
            Terminator { source_info, kind: TerminatorKind::Drop { place, target, unwind }, pta_target: false } => {
...
fn insert_term_block<'tcx>(body: &mut Body<'tcx>, kind: TerminatorKind<'tcx>) -> BasicBlock {
    ...
    body.basic_blocks_mut().push(BasicBlockData {
        statements: Vec::new(),
        terminator: Some(Terminator { source_info, kind, pta_target: false }),
        is_cleanup: false,
    })
}
...
fn insert_panic_block<'tcx>(
    ...
    body.basic_blocks_mut().push(BasicBlockData {
        statements: Vec::new(),
        terminator: Some(Terminator { source_info, kind: term, pta_target: false }),
        is_cleanup: false,
    });
...
fn create_generator_resume_function<'tcx>(
    ...
    if can_unwind {
        ...
        let poison_block = body.basic_blocks_mut().push(BasicBlockData {
            ...
            terminator: Some(Terminator { source_info, kind: TerminatorKind::Resume, pta_target: false }),
            is_cleanup: true,
        });
            ...
            operation.target_block(point).map(|target| {
                ...
                for i in 0..(body.local_decls.len()) {
                    ...
                    if needs_storage_live {
                        statements
                            .push(Statement { source_info, kind: StatementKind::StorageLive(l), pta_target: false });
                    }
                }
