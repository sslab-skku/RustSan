...
fn run_optimization_passes<'tcx>(tcx: TyCtxt<'tcx>, body: &mut Body<'tcx>) {
    ...
    // MyDbg Pass: mark unsafe instruction
    if tcx.sess.opts.pta_filter || tcx.sess.opts.unsafe_to_ir {
        pm::run_passes(
            tcx,
            body,
            &[&mydbg::MyDbg]
    ...
    // PTAFilter Pass: mark unsafe local
    if tcx.sess.opts.pta_filter {
        pm::run_passes(
            tcx,
            body,
            &[&pta_filter::PTAFilter]
        );
    }

    if tcx.sess.opts.mydbg {
        pm::run_passes(
            tcx,
            body,
            &[&mydbg::MyDbg]
...
fn optimized_mir<'tcx>(tcx: TyCtxt<'tcx>, did: DefId) -> &'tcx Body<'tcx> {
    ...
    let mut result = tcx.arena.alloc(inner_optimized_mir(tcx, did));
    if result.did.is_none() {
        result.did = Some(did.to_def_id());
    }
    result
}
