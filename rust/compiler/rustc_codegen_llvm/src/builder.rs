...
impl<'a, 'll, 'tcx> BuilderMethods<'a, 'tcx> for Builder<'a, 'll, 'tcx> {
    fn safety(&mut self)->bool {return self.safety}
    fn set_safety(&mut self, safety:bool) {self.safety = safety}
    fn set_unsafe_args(&mut self, args: u32) {self.unsafe_args = args}
    fn get_unsafe_args(&self)->u32 {self.unsafe_args}
    fn set_allocate_method_attribute(&mut self) {
        unsafe {
            let attr = llvm::CreateAttrStringValue(self.cx().llcx, "AllocaMethod", "AllocaMethod");
            attributes::apply_to_llfn(self.llfn(), crate::llvm::AttributePlace::Function, &[attr]);
        }
    }
    ...
    fn invoke(
        ...
        /*
        if self.cx().tcx.sess.opts.mydbg {
            println!("fn: {:?}, args: {:?}, safety: {}", llfn, args, self.safety());
        }
        */
    ...
    fn alloca(&mut self, ty: &'ll Type, align: Align) -> &'ll Value {
        ...
        unsafe {
            let alloca = match self.safety() {
                // true => llvm::LLVMBuildUAlloca(bx.llbuilder, ty, UNNAMED),
                true => llvm::LLVMBuildAlloca(bx.llbuilder, ty, UNNAMED),
                false => llvm::LLVMBuildAlloca(bx.llbuilder, ty, UNNAMED),
            };
    ...
    fn struct_gep(&mut self, ty: &'ll Type, ptr: &'ll Value, idx: u64) -> &'ll Value {
        ...
        unsafe { 
            let gep = llvm::LLVMBuildStructGEP2(self.llbuilder, ty, ptr, idx as c_uint, UNNAMED);
            self.insert_unsafe_metadata(gep);
            gep
        }
    }
    ...
    fn trunc(&mut self, val: &'ll Value, dest_ty: &'ll Type) -> &'ll Value {
        unsafe { 
            let trunc = llvm::LLVMBuildTrunc(self.llbuilder, val, dest_ty, UNNAMED);
            self.insert_unsafe_metadata(trunc);
            trunc
        }
    }
    ...
    fn sext(&mut self, val: &'ll Value, dest_ty: &'ll Type) -> &'ll Value {
        unsafe { 
            let sext = llvm::LLVMBuildSExt(self.llbuilder, val, dest_ty, UNNAMED);
            self.insert_unsafe_metadata(sext);
            sext
        }
    }
    ...
    fn ptrtoint(&mut self, val: &'ll Value, dest_ty: &'ll Type) -> &'ll Value {
        unsafe {
            let ptrtoint = llvm::LLVMBuildPtrToInt(self.llbuilder, val, dest_ty, UNNAMED);
            self.insert_unsafe_metadata(ptrtoint);
            ptrtoint
        }
    }
    ...
    fn inttoptr(&mut self, val: &'ll Value, dest_ty: &'ll Type) -> &'ll Value {
        unsafe { 
            let inttoptr = llvm::LLVMBuildIntToPtr(self.llbuilder, val, dest_ty, UNNAMED);
            self.insert_unsafe_metadata(inttoptr);
            inttoptr
        }
    }
    ...
    fn bitcast(&mut self, val: &'ll Value, dest_ty: &'ll Type) -> &'ll Value {
        unsafe {
            let bitcast = llvm::LLVMBuildBitCast(self.llbuilder, val, dest_ty, UNNAMED);
            self.insert_unsafe_metadata(bitcast);
            bitcast
        }
    }
    ...
    fn intcast(&mut self, val: &'ll Value, dest_ty: &'ll Type, is_signed: bool) -> &'ll Value {
        unsafe { 
            let intcast = llvm::LLVMRustBuildIntCast(self.llbuilder, val, dest_ty, is_signed);
            self.insert_unsafe_metadata(intcast);
            intcast
        }
    }
    ...
    fn pointercast(&mut self, val: &'ll Value, dest_ty: &'ll Type) -> &'ll Value {
        unsafe {
            let pointercast = llvm::LLVMBuildPointerCast(self.llbuilder, val, dest_ty, UNNAMED);
            self.insert_unsafe_metadata(pointercast);
            pointercast
        }
    }
    ...
    fn icmp(&mut self, op: IntPredicate, lhs: &'ll Value, rhs: &'ll Value) -> &'ll Value {
        ...
        unsafe {
            let icmp = llvm::LLVMBuildICmp(self.llbuilder, op as c_uint, lhs, rhs, UNNAMED);
            self.insert_unsafe_metadata(icmp);
            icmp
        }
    }
    ...
    fn extract_value(&mut self, agg_val: &'ll Value, idx: u64) -> &'ll Value {
        assert_eq!(idx as c_uint as u64, idx);
        unsafe { 
            let extract = llvm::LLVMBuildExtractValue(self.llbuilder, agg_val, idx as c_uint, UNNAMED) ;
            self.insert_unsafe_metadata(extract);
            extract 
        }
    }
    ...
    fn insert_value(&mut self, agg_val: &'ll Value, elt: &'ll Value, idx: u64) -> &'ll Value {
        assert_eq!(idx as c_uint as u64, idx);
        unsafe { 
            let insert = llvm::LLVMBuildInsertValue(self.llbuilder, agg_val, elt, idx as c_uint, UNNAMED);
            self.insert_unsafe_metadata(insert);
            insert 
        }
    }
    ...
    fn atomic_rmw(
        ...
        unsafe {
            let value = llvm::LLVMBuildAtomicRMW(
                self.llbuilder,
            ...
            if !fn_abi.c_variadic{
                if self.safety(){
                    unsafe {
                        let key = "FFI";
                        let kind = llvm::LLVMGetMDKindIDInContext(self.cx.llcx, key.as_ptr() as *const c_char, key.len() as c_uint);
                        llvm::LLVMSetMetadata(call, kind, llvm::LLVMMDNodeInContext(self.cx.llcx, ptr::null(), 0));
                    }
                }
            }
        }
...
impl<'a, 'll, 'tcx> Builder<'a, 'll, 'tcx> {
    fn with_cx(cx: &'a CodegenCx<'ll, 'tcx>) -> Self {
        ...
        Builder { llbuilder, cx , safety:false, unsafe_args:0 }
    }
